class PositionVec2 {
  ...
  draw(ctx) {
    ctx.beginPath();
    ctx.arc(this.x, this.y, this.color === "#fff" ? 1.5 : 3, 0, 2 * Math.PI);
...
class PositionUtil {
  ...
  static colorQueriedAgents(found, color) {
    for (let agent of found) {
      agent.color = color;
    }
  }

  static clearPreviousSearch(searchBox, foundAgents) {
    if (searchBox && foundAgents) {
      searchBox = null;
      PositionUtil.colorQueriedAgents(foundAgents, "#fff");
      foundAgents = null;
    }
  }
}
